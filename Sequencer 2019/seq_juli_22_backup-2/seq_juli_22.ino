...
void setup() {
  ...
  //Read the step patterns from the eeprom
  for(int track = 0; track<4; track ++){
	    byte parameter = 0; //8 parameters / cells for each track: 0 steps, 1 length, 2 selected track
		eeprom_offset += 16 * parameter; //add an offset to make room in the eeprom for all the parameters
		int eeprom_address = eeprom_offset * track;
		for(int i=0;i<8;i++){
			seq[track][i] = EEPROM.read(track * eeprom_address + i); //read the recorded settings for each step on each track
		}
...
void loop() {
 ...
 trigger_with_and(i,i,count[i]);
  ...
  //int repeats = 3;
  settings_eeprom(number_of_tracks); //temporarily disabled!!!!
...
int settings_eeprom(int tracks){
 ...
 int eeprom_offset = 8;//128; //128 bytes for each track
...
void track_selector(){ 
  for(int i=0;i<number_of_tracks;i++){ //read through the step switches 
  track_selector_state = digitalRead(track_selector_switch);
  buttonState[i] = digitalRead(buttonPin[i]);  // read the pushbutton input pin:

  if (digitalRead(track_selector_switch) == pressed && buttonState[i] == pressed && debounce_f(time2, debounce_var)) {   // compare the buttonState to its previous state and see if it is pressed
    selected_track = i; 
    time2 = millis();    
  }
